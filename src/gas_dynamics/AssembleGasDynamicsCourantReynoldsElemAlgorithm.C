...
// Class Definition
//==========================================================================
// AssembleGasDynamicsCourantReynoldsElemAlgorithm - GasDynamicsCourant number calc for both edge
//                                        and elem
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
AssembleGasDynamicsCourantReynoldsElemAlgorithm::AssembleGasDynamicsCourantReynoldsElemAlgorithm(
  Realm &realm,
  stk::mesh::Part *part)
...
//--------------------------------------------------------------------------
void
AssembleGasDynamicsCourantReynoldsElemAlgorithm::execute()
{
  ...
  std::vector<double> ws_speedOfSound;
  ...
  for ( const stk::mesh::Bucket* bucket_ptr : elem_buckets ) {
    ...
    ws_speedOfSound.resize(nodesPerElement);
    ...
    double *p_speedOfSound = &ws_speedOfSound[0];
    ...
    for ( stk::mesh::Bucket::size_type k = 0 ; k < length ; ++k ) {
      ...
      for ( int ni = 0; ni < num_nodes; ++ni ) {
        ...
        p_speedOfSound[ni] = *stk::mesh::field_data(*speedOfSound_, node);
      ...
      for ( int ip = 0; ip < numScsIp; ++ip ) {
        ...
        udotx = std::abs(udotx) + 0.5*(p_speedOfSound[ir]+p_speedOfSound[il])*std::sqrt(dxSq);
