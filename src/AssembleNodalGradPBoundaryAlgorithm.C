...
// Class Definition
//==========================================================================
// AssembleNodalGradPBoundaryAlgorithm - adds in pressure boundary 
//                                       contribution for elem/edge proj 
//                                       nodal gradient
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
AssembleNodalGradPBoundaryAlgorithm::AssembleNodalGradPBoundaryAlgorithm(
  Realm &realm,
  stk::mesh::Part *part,
...
//--------------------------------------------------------------------------
void
AssembleNodalGradPBoundaryAlgorithm::execute()
{
  ...
  for ( stk::mesh::BucketVector::const_iterator ib = face_buckets.begin();
    ...
    for ( stk::mesh::Bucket::size_type k = 0 ; k < length ; ++k ) {
      ...
      const double * dynamicP = stk::mesh::field_data(*dynamicPressure, b, k);
      ...
      for ( int ip = 0; ip < numScsIp; ++ip ) {
        ...
        double pIp = -dynamicP[ip];
        const int offSet = ip*nodesPerFace;
        for ( int ic = 0; ic < nodesPerFace; ++ic ) {
          pIp += p_shape_function[offSet+ic]*p_pressure[ic];
        }
        ...
        for ( int j = 0; j < nDim; ++j ) {
          double fac = pIp*areaVec[ip*nDim+j];
          gradPNN[j] += fac*inv_volNN;
        }
      }
    }
  }
