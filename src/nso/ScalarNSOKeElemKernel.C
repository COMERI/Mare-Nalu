...
ScalarNSOKeElemKernel<AlgTraits>::ScalarNSOKeElemKernel(
...
template<typename AlgTraits>
void
ScalarNSOKeElemKernel<AlgTraits>::execute(
  ...
  ScratchViews<DoubleType>& scratchViews)
{
  ...
  for ( int ip = 0; ip < AlgTraits::numScsIp_; ++ip ) {
    ...
    const DoubleType nu = stk::math::min(Cupw_*nuFirstOrder, nuResidual/turbCoeff_);
