...
VolumeOfFluidEvaporationElemKernel<AlgTraits>::VolumeOfFluidEvaporationElemKernel(
    ...
    ipNodeMap_(sierra::nalu::MasterElementRepo::get_volume_master_element(AlgTraits::topo_)->ipNodeMap())
{
  ...
  // provide output
  NaluEnv::self().naluOutputP0() << "Evaporation constants jm/m/n/c: " 
                                 << jm_ << "/" << m_ << "/" << n_ << "/" << c_ << std::endl;
}
...
template<typename AlgTraits>
void
VolumeOfFluidEvaporationElemKernel<AlgTraits>::execute(
  ...
  ScratchViews<DoubleType>& scratchViews)
{
  ...
  for ( int ip = 0; ip < AlgTraits::numScvIp_; ++ip ) {
    ...
    // assemble rhs (no RHS)
    rhs(nearestNode) -= c_*sM/rhoL_*v_scv_volume(ip);
  }
}

INSTANTIATE_KERNEL(VolumeOfFluidEvaporationElemKernel);
