...
TurbKineticEnergyKEpsilonSrcElemKernel<AlgTraits>::TurbKineticEnergyKEpsilonSrcElemKernel(
...
template<typename AlgTraits>
void
TurbKineticEnergyKEpsilonSrcElemKernel<AlgTraits>::execute(
  ...
  ScratchViews<DoubleType>& scratchViews)
{
  ...
  for (int ip=0; ip < AlgTraits::numScvIp_; ++ip) {
    ...
    // everything lives at the quadrature points
    ...
    for (int ic = 0; ic < AlgTraits::nodesPerElement_; ++ic) {
      lhs(nearestNode, ic) += v_shape_function_(ip,ic)*rhoIp*epsIp/tkeIpC*scV;
    }
  }
} 
  
INSTANTIATE_KERNEL(TurbKineticEnergyKEpsilonSrcElemKernel);
